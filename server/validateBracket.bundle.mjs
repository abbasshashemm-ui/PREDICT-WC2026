...
function updateGroupMatchScore(state, matchId, userHomeScore, userAwayScore) {
  const target = state.groupMatches.find((m) => m.id === matchId);
  if (!target) return state;
  const groupMatches = state.groupMatches.map(
    (m) => m.id === matchId ? {
      ...m,
      userHomeScore,
      userAwayScore,
...
function setManualTieBreakOrder(state, groupId, orderedTeamIds) {
  const manualTieBreakOrders = {
    ...state.manualTieBreakOrders,
...
function updateKnockoutMatchScore(state, matchId, userHomeScore, userAwayScore, penaltyWinnerId) {
  const target = state.knockoutMatches.find((m) => m.id === matchId);
  if (!target) return state;
  let knockoutMatches = state.knockoutMatches.map((m) => {
    if (m.id !== matchId) return m;
    const isDraw = userHomeScore !== null && userAwayScore !== null && userHomeScore === userAwayScore;
    return {
      ...m,
      userHomeScore,
      userAwayScore,
...
// src/logic/validateBracket.ts
var BracketValidationError = class extends Error {
  code;
...
var MIN_SCORE = 0;
var MAX_SCORE = 99;
var KNOCKOUT_MATCH_COUNT = 32;
var VALID_TEAM_IDS = new Set(createTeams().map((team) => team.id));
function isPlainObject(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
function parseRawScoreRow(value, index) {
  ...
  if (!Number.isInteger(homeScore) || !Number.isInteger(awayScore) || homeScore < MIN_SCORE || awayScore < MIN_SCORE || homeScore > MAX_SCORE || awayScore > MAX_SCORE) {
    throw new BracketValidationError(
      "INVALID_SCORE",
      `scores[${index}] scores must be integers between ${MIN_SCORE} and ${MAX_SCORE}`
    );
  }
  if (penaltyWinnerId !== void 0 && penaltyWinnerId !== null) {
...
function parseClaimedSlot(value, index) {
  if (!isPlainObject(value)) {
    throw new BracketValidationError(
      "INVALID_PAYLOAD",
  ...
  const parseTeamId = (teamId, field) => {
...
function parsePayload(payload) {
  ...
  let manualTieBreakOrders;
  if (payload.manualTieBreakOrders !== void 0) {
...
function assertUniqueMatchIds(scores) {
...
function extractKnockoutLayout(knockoutMatches) {
  return [...knockoutMatches].sort((a, b) => a.matchId - b.matchId).map((match) => ({
...
function slotsEqual(a, b) {
  return a.matchId === b.matchId && a.homeTeamId === b.homeTeamId && a.awayTeamId === b.awayTeamId;
}
function compareKnockoutLayouts(computed, claimed) {
  ...
  for (const expected of computed) {
    ...
    if (!slotsEqual(expected, actual)) {
      throw new BracketValidationError(
        "CLAIMED_LAYOUT_MISMATCH",
        `Knockout slot mismatch at match ${expected.matchId}: server computed ${expected.homeTeamId ?? "TBD"} vs ${expected.awayTeamId ?? "TBD"}, client claimed ${actual.homeTeamId ?? "TBD"} vs ${actual.awayTeamId ?? "TBD"}`
      );
    }
  }
...
function recomputeBracketFromScores(scores, manualTieBreakOrders) {
  ...
  for (const row of sorted) {
    ...
    if (template.phase === "knockout") {
      ...
      if (isDraw && row.penaltyWinnerId && row.penaltyWinnerId !== live.homeTeamId && row.penaltyWinnerId !== live.awayTeamId) {
        throw new BracketValidationError(
          "INVALID_PENALTY_WINNER",
...
function validateBracketSubmission(payload) {
...
function validateBracketSubmissionSafe(payload) {
  try {
...
export {
  ...
  extractKnockoutLayout,
  recomputeBracketFromScores,
  validateBracketSubmission,
  validateBracketSubmissionSafe
};
